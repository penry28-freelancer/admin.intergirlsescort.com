  ...
  methods: {
    async getList() {
      try {
        ...
        const { data } = await escostReportResource.list(this.table.listQuery);
    ...
    onDestroy(id) {
      this.$confirm(this.$t('confirms.permanently_delete.singular', {
        ...
        try {
          this.table.loading = true;
          await escostReportResource.destroy(id);
    ...
    onToggleVerify(id) {
      this.$confirm(this.$t('confirms.toggle_verify', {
        ...
        try {
          this.table.loading = true;
          const status = await escostReportResource.toggleVerify(id);
          const idxRecord = this.table.list.findIndex(item => item.id === id);
