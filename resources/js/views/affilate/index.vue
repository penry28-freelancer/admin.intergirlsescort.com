<template>
  <div class="page-target">
    <table-panel>
      <template slot="title">
        <small class="text--uppercase">{{ $t('table.title.affilate') }}</small>
      </template>
      ...
      <template slot="table">
        ...
        >
          ...
          <el-table-column :label="$t('table.common.full_name')" prop="full_name" sortable="custom" width="300">
            <template slot-scope="{ row }">
          ...
          <el-table-column :label="$t('table.common.is_verify')" prop="is_verify" sortable="custom" width="200">
            <template slot-scope="{ row }">
              <el-button-group>
                <el-button size="mini" :type="transferVerifyData(row.is_verify).btnColor" :icon="transferVerifyData(row.is_verify).icon" />
              </el-button-group>
            </template>
          </el-table-column>
...
import AffilateResource from '@/http/api/v1/affilate';
const affilateResource = new AffilateResource();
  ...
  methods: {
    async getList() {
      try {
        ...
        const { data } = await affilateResource.list(this.table.listQuery);
    ...
    onDestroy(id) {
      this.$confirm(this.$t('confirms.permanently_delete.singular', {
        ...
        try {
          this.table.loading = true;
          await affilateResource.destroy(id);
