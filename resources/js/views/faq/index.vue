<template>
  <div class="page-target">
    <table-panel>
      <template slot="title">
        <small class="text--uppercase">{{ $t('table.title.faq') }}</small>
      </template>

      <template slot="tools">
        <el-button type="primary" size="mini" class="text--uppercase" @click="onOpenForm">
          {{ $t('action.add', { model: $t('model.faq') }) }}
        </el-button>
      </template>
      ...
      <template slot="table">
        ...
        >
          ...
          <el-table-column :label="$t('table.common.question')" prop="question" sortable="custom" width="500">
            <template slot-scope="{ row }">
              <div class="heading" v-html="row.question" />
            </template>
          </el-table-column>

          <el-table-column :label="$t('table.common.answer')" prop="answer" sortable="custom" width="650">
            <template slot-scope="{ row }">
              <div class="heading" v-html="row.answer" />
            </template>
          </el-table-column>

          <el-table-column :label="$t('table.common.type')" prop="type" sortable="custom" width="200">
            <template slot-scope="{ row }">
              <div class="heading">
                {{ serverConfig.faqType.label[row.type] }}
              </div>
            </template>
          </el-table-column>
  ...
  methods: {
    async getList() {
      try {
        ...
        const { data } = await faqResource.list(this.table.listQuery);
