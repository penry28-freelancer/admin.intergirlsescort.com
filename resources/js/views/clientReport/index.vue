<template>
  <div class="page-target">
    <table-panel>
      <template slot="title">
        <small class="text--uppercase">{{ $t('table.title.client_report') }}</small>
      </template>
      ...
      <template slot="table">
        ...
        >
          ...
          <el-table-column :label="$t('table.common.email')" prop="email" sortable="custom" width="200">
            ...
            <el-table-column :label="$t('table.common.verified_at')" prop="verified_at" sortable="custom" width="200">
              <template v-if="row.verified_at" slot-scope="{ row }">
                <div class="value d-flex align-center">
                  <svg-icon icon-class="date" />
          ...
          <el-table-column align="center" header-align="center" :label="$t('table.common.action')">
            <template slot-scope="{ row }">
              <el-button-group>
                <el-button size="mini" @click="readContentHandler(row.description)">{{ $t('button.read_message') }}</el-button>
                <el-button size="mini" icon="el-icon-delete" @click="onDestroy(row.id)" />
                <el-tooltip class="item" effect="dark" :content="transferVerifyData(row.verified_at).tooltip" placement="top">
                  <el-button size="mini" :type="transferVerifyData(row.verified_at).btnColor" :icon="transferVerifyData(row.verified_at).icon" @click="onToggleVerify(row.id)" />
                </el-tooltip>
              </el-button-group>
            </template>
          </el-table-column>
    ...
    <el-dialog
      :title="$t('title_dialog.description')"
...
import ClientReportResource from '@/http/api/v1/clientReport';
const clientReportResource = new ClientReportResource();
  ...
  data: () => ({
    ...
    table: {
      listQuery: {
        ...
        page: 1,
        withOrWhere: ['type', CONST_REPORT.client],
        search: '',
        orderBy: 'updated_at',
  ...
  methods: {
    async getList() {
      try {
        ...
        const { data } = await clientReportResource.list(this.table.listQuery);
    ...
    onDestroy(id) {
      this.$confirm(this.$t('confirms.permanently_delete.singular', {
        ...
        try {
          this.table.loading = true;
          await clientReportResource.destroy(id);
    ...
    onToggleVerify(id) {
      this.$confirm(this.$t('confirms.toggle_verify', {
        ...
        try {
          this.table.loading = true;
          const status = await clientReportResource.toggleVerify(id);
          const idxRecord = this.table.list.findIndex(item => item.id === id);
