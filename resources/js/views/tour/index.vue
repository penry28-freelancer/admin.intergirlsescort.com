<template>
    <div class="page-target">
      <table-panel>
        <template slot="title">
          <small class="text--uppercase">{{ $t('table.title.tour') }}</small>
        </template>

        <template slot="tools">
          <el-button type="primary" size="mini" class="text--uppercase" @click="onOpenForm">
            {{ $t('action.add', { model: $t('model.tour') }) }}
          </el-button>
        </template>
        ...
        <template slot="table">
          ...
          >
            ...
            <el-table-column :label="$t('table.common.title')" prop="name" sortable="custom" width="200">
              <template slot-scope="{ row }">
                <div class="heading">{{ row.title }}</div>
              </template>
            </el-table-column>

            <el-table-column :label="$t('table.common.start_date')" prop="name" sortable="custom" width="200">
              <template slot-scope="{ row }">
                  <div class="value d-flex align-center">
                    <svg-icon icon-class="date" />
            ...
            <el-table-column :label="$t('table.common.end_date')" prop="name" sortable="custom" width="200">
              <template slot-scope="{ row }">
                  <div class="value d-flex align-center">
                    <svg-icon icon-class="date" />
    ...
    methods: {
      async getList() {
        try {
          ...
          const { data } = await tourResource.list(this.table.listQuery);
